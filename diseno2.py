...
if read_file==True:
    ...
    Z0=np.array(AF[:,2])+B[2]
    vx0=np.array(AF[:,3])+B[3]
    vy0=np.array(AF[:,4])+B[4]
    vz0=np.array(AF[:,5])+B[5]
    NParticles=len(X0)
    AF=0
    z0ave=0.08
...
if True :
    ...
    run_length = 0.18  # the lenght of the simulation 
...
# SOLENOID DEFINITION
    ...
    #input()
    wp.addnewsolenoid(zi=solenoid_zi[0]-z0ave,
                zf=solenoid_ze[0]-z0ave,
                ri=solenoid_radius,
                maxbz=0.062)  # 0.062 .07

    solenoid_zi[1]=solenoid_zi[0]+0.08
    solenoid_ze[1]=solenoid_ze[0]+0.08
    wp.addnewsolenoid(zi=solenoid_zi[1]-z0ave,
                zf=solenoid_ze[1]-z0ave,
                ri=solenoid_radius,
                maxbz=1.0*mag_solenoid)  # 0.075 T for p+, 0.125 T for H2+
    
#0.0511  0.0539

    wp.addnewsolenoid(zi=solenoid_zi[1]+0.1-z0ave,
                zf=solenoid_zi[1]+0.1+0.01-z0ave,
...
#0.0575 0.06039
    ...
    pipe = wp.ZCylinderOut(radius=solenoid_radius*1.0, zlower=0.0-z0ave, zupper=solenoid_ze[0]-z0ave)
    pipe1 = wp.ZCylinderOut(radius=0.003, zlower=solenoid_zi[1]-z0ave, zupper=solenoid_zi[1]+0.005-z0ave)
    pipe2 = wp.ZCylinderOut(radius=0.003, zlower=solenoid_zi[2]-z0ave, zupper=solenoid_zi[2]+0.004-z0ave)
    pipe3 = wp.ZCylinderOut(radius=0.004, zlower=solenoid_ze[2]-z0ave, zupper=solenoid_ze[2]+0.004-z0ave)
