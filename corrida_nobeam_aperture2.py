...
if True :
    ...
    run_length = 0.375  # the lenght of the simulation 
    ...
    i_beam = 0.0050 * wp.mA     # beam current per species [A]
...
# SOLENOID DEFINITION
    ...
    aperture_plate2 = wp.ZCylinderOut(radius=0.002, zlower=plate_zcenter2-plate_thickness/2, zupper=plate_zcenter2+plate_thickness/2)
    aperture_plate3 = wp.ZCylinderOut(radius=0.002, zlower=plate_zcenter3-plate_thickness/2, zupper=plate_zcenter3+plate_thickness/2)

    # --- Pipe in the solenoid transport
    pipe = wp.ZCylinderOut(radius=0.025, zlower=0, zupper=run_length-0.005, voltage=0)

    # Combine all conductors now, but install them only after wp.generate(),
    # when Warp has initialized the mesh decomposition.
    conductors = aperture_plate + aperture_plate2 + aperture_plate3 + pipe
